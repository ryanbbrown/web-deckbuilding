...
export const testStorages = async (_tc) => {
  ...
  try {
    const files = await s3.client
      .listObjectsV2(s3TestBucketName, '', true)
      .toArray();
    await s3.client.removeObjects(
      s3TestBucketName,
      files.map((file) => file.name)
    );
  } catch (e) {}
  const postgres = await createPostgresStorage({
    database: env.ensureConf('postgres-testdb'),
  });
  ...
  for (const storageName in storages) {
    ...
    await t.groupAsync(`storage: ${storageName}`, async () => {
      ...
      {
        const sv = await storage.retrieveStateVector(
          'nonexistend',
          'nonexistend'
        );
        t.assert(sv === null);
